...
const MenuBar = ({ editor, imageInputRef }: { editor: any, imageInputRef: React.RefObject<HTMLInputElement> }) => {
    ...
    const getCurrentHeadingLabel = () => {
        for (let i = 1; i <= 4; i++) {
            if (editor.isActive('heading', { level: i })) return `หัวข้อ ${i}`;
        }
        return "ปกติ";
    };
    ...
    const getCurrentFont = () => {
        ...
        if (!attrs.fontFamily) return "ค่าเริ่มต้น";
        const found = FONT_FAMILIES.find(f => f.value === attrs.fontFamily);
        return found ? found.label : "ค่าเริ่มต้น";
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-10 pb-8 border-b border-slate-200 dark:border-slate-800">
                <div className="flex items-start">
                    ...
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">เพิ่มหลักสูตรใหม่</h1>
                        <p className="text-sm text-slate-500 mt-1">ระบุข้อมูลหลักสูตรและระดับการศึกษาที่เปิดสอน</p>
                    </div>
                </div>
                <button 
                    onClick={handleSave} 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-6">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                        ...
                        <input 
                            type="text" 
                            placeholder="พิมพ์ชื่อหลักสูตรที่นี่..." 
                            value={name} 
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                        <div className="px-8 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">รายละเอียดหลักสูตร</span>
                        </div>
                        <style>{`
                            .tiptap-editor {
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6">
                            <div 
                                ...
                                className="aspect-[16/10] bg-slate-50 dark:bg-slate-800 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center cursor-pointer overflow-hidden relative group"
                            >
                                {imagePreview ? (
                                    <img src={imagePreview} className="w-full h-full object-cover" alt="Preview" />
                                ) : (
                                    <>
                                        <PlusSquare className="text-slate-300 mb-2" size={32} />
                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">เลือกรูปภาพ</span>
                                    </>
                                )}
                            </div>
                        ...
                        <div className="p-6">
                            ...
                            >
                                ...
                                <span className="text-xs font-bold truncate">
                                    {document ? document.name : "คลิกเพื่อเลือกไฟล์ (PDF, Doc, Zip)"}
                                </span>
                                {document && <X size={16} className="ml-auto shrink-0" onClick={(e) => { e.stopPropagation(); setDocument(null); }} />}
                            </div>
