...
export default function NewsEdit() {
        ...
        const fetchNews = async () => {
            ...
            try {
                ...
                if (data.published_at) {
                    setPublishedAt(data.published_at.split('T')[0]);
                }
                setMetaTitle(data.meta_title || "");
                setMetaDesc(data.meta_description || "");
                setMetaKeywords(data.meta_keywords || "");
                if (data.featured_image) {
                    setImagePreview(`${import.meta.env.VITE_API_URL}/storage/${data.featured_image}`);
                }
    ...
    const handleUpdate = async () => {
        ...
        const loadingToast = toast.loading("กำลังอัปเดตข้อมูลข่าวสาร...");
        ...
        try {
            ...
            if (res.ok) {
                toast.success("อัปเดตข้อมูลข่าวสารสำเร็จ", { id: loadingToast });
                navigate("/admin/news");
            } else if (res.status === 422) {
                toast.error("ข้อมูลไม่ถูกต้อง", { id: loadingToast });
                setValidationErrors(data.errors || {});
            } else {
                toast.error(data.message || "อัปเดตไม่สำเร็จ", { id: loadingToast });
            }
        } catch (err) {
    ...
    if (loading) return <div className="p-20 text-center text-slate-400 font-bold uppercase tracking-widest text-xs animate-pulse">กำลังโหลดข้อมูล...</div>;
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-8 pb-8 border-b border-slate-200 dark:border-slate-800">
                <div className="flex items-start">
                    ...
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">แก้ไขข้อมูลข่าวสาร</h1>
                        <p className="text-sm text-slate-500 mt-1">อัปเดตข้อมูลข่าวสารที่ต้องการเผยแพร่สู่สาธารณะ</p>
                    </div>
                </div>
                <div className="flex items-center space-x-3 mt-6 md:mt-0">
                    ...
                    <button onClick={handleUpdate} className="px-6 py-2 bg-primary-600 text-white rounded-lg text-xs font-bold hover:bg-primary-700 transition-all shadow-md shadow-primary-600/20 flex items-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-6">
                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                        <div className="px-8 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">หัวข้อข่าวสาร</span>
                        </div>
                        <input type="text" value={title} onChange={handleTitleChange} className="w-full px-8 py-6 text-2xl font-bold bg-transparent border-none focus:ring-0 text-slate-800 dark:text-white outline-none" />
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6">
                            <div onClick={() => document.getElementById('edit-featured-upload-vfinal')?.click()} className="aspect-[16/9] bg-slate-50 dark:bg-slate-800 rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center cursor-pointer overflow-hidden relative group">
                                {imagePreview ? <img src={imagePreview} className="w-full h-full object-cover" alt="Preview" /> : <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">เลือกไฟล์รูปภาพ</span>}
                            </div>
                            <input type="file" id="edit-featured-upload-vfinal" className="hidden" accept="image/*" onChange={(e) => {
                                const file = e.target.files?.[0];
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6 space-y-4">
                            <div 
                                onClick={() => document.getElementById('edit-gallery-upload-vfinal')?.click()} 
                            ...
                            >
                                ...
                                <span className="text-[10px] font-bold text-slate-400 group-hover:text-slate-500 uppercase tracking-widest transition-colors">เพิ่มรูปภาพในคลังภาพ</span>
                                <span className="text-[8px] text-slate-400 mt-1 font-semibold">คลิกเพื่อเลือกไฟล์รูปภาพเพิ่มเติม</span>
                            </div>
                            <input 
                                type="file" 
                                id="edit-gallery-upload-vfinal" 
                                className="hidden" 
                                accept="image/*" 
                                multiple 
                            ...
                            {(existingGallery.length > 0 || newGalleryPreviews.length > 0) && (
                                <div className="grid grid-cols-3 gap-2.5 pt-2">
                                    ...
                                    {existingGallery.map((path, idx) => (
                                        <div key={`existing-${idx}`} className="aspect-square bg-slate-100 dark:bg-slate-800 rounded-lg overflow-hidden relative group border border-slate-100 dark:border-slate-700 shadow-sm">
                                            <img src={`${import.meta.env.VITE_API_URL}/storage/${path}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" alt={`Existing ${idx + 1}`} />
                                            <button 
                                                type="button"
                                                ...
                                                title="ลบรูปภาพนี้จากเซิร์ฟเวอร์"
                                            >
                                                <X size={12} />
                                            </button>
                                            <span className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded bg-slate-900/80 text-[8px] text-slate-300 font-bold tracking-wider">เซิร์ฟเวอร์</span>
                                        </div>
                                    ))}

                                    {/* Newly Added Gallery Images */}
                                    {newGalleryPreviews.map((previewUrl, idx) => (
                                        <div key={`new-${idx}`} className="aspect-square bg-slate-100 dark:bg-slate-800 rounded-lg overflow-hidden relative group border border-slate-100 dark:border-slate-700 shadow-sm animate-fade-in">
                                            <img src={previewUrl} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" alt={`New ${idx + 1}`} />
                                            <button 
                                                type="button"
                                            ...
                                            <span className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded bg-primary-600/95 text-[8px] text-white font-bold tracking-wider">ใหม่</span>
                                        </div>
                                    ))}
                                </div>
                            )}
